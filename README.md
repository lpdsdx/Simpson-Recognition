...
构建卷积神经网络，带有6个ReLU激活函数的卷积层及3个池化层和1个全连接层，之所以构造6个卷积层，首先是因为辛普森一家里的人物确实长得都太像了，稍不留神就成了脸盲，其次是人物的形态不一，人脸分布在不同的区域，所以要更加细致的提取特征，以达到区分的能力。卷积池化层还增加了Dropout，可以有效防止过拟合，输出层采用softmax函数来输出各类概率，优化器optimizer选用随机梯度下降SGD，学习率lr=0.01，学习率衰减decay=1e-6，动量momentum=0.9，动量是为了越过平坦区域或泥石流区域(左右震荡)。
